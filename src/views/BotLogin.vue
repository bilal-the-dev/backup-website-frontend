    ...
    >
      <div class="text-center">
        <h1 class="text-3xl font-semibold text-white">
          DiscArchive - Bot Login
        </h1>
        <p class="text-sm text-gray-400 mt-2">
          Backup your DMs, servers, and messages locally using your bot.
        </p>
      </div>
...
const fetchConfig = async () => {
  try {
    const result = await sendGetRequest("config?tokenType=bot");

    if (Array.isArray(result?.data) && result.data.length > 0) {
      // Try to find a bot token
      const tokenObj = result.data.find((item) => item.tokenType === "bot");
      ...
      if (tokenObj && tokenObj.token) {
        ...
        // Set botLoggedIn = true
        localStorage.setItem("botLoggedIn", "true");

        // Only call /users/@me if we got a bot token
        try {
          const userResult = await sendGetRequest("users/@me?tokenType=bot");
          if (userResult?.data) {
            user.value.name = userResult.data.username;
            user.value.avatar = userResult.data.avatarURL;
          }
        } catch (err) {
          toast.error(err.message || "Failed to fetch bot info");
        }
        return; // exit early since bot token exists
      }
    }

    // If no bot token found, set botLoggedIn = false
    localStorage.setItem("botLoggedIn", "false");
...
const continueAction = async () => {
  ...
  const payload = {
    token: token.value,
    tokenType: "bot", // always bot
  };

  try {
    const result = await sendPostRequest("config?tokenType=bot", payload);
    ...
    toast.error(err.message || "Failed to save bot configuration");
    localStorage.setItem("botLoggedIn", "false");
  } finally {
    loading.value = false;
  }
