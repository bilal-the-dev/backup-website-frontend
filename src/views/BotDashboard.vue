      ...
      <div class="mb-10">
        ...
        >
          ...
          <div class="relative px-10 py-10 flex flex-col gap-4">
            ...
            >
              Bot Backup Dashboard
            </h1>

            <p
              class="text-gray-400 max-w-2xl text-base md:text-lg leading-relaxed"
            >
              Securely backup your Discord servers. Everything stays local —
...
const fetchUser = async () => {
  try {
    const result = await sendGetRequest("users/@me?tokenType=bot");
...
const backup = async (item) => {
  try {
    ...
    const endpoint = `/backup/${item.id}?tokenType=bot`;

    // Prepare payload for the API
    const payload = {
      itemType: "guild", // fixed type
...
async function download(server) {
  try {
    ...
    const response = await sendGetRequest(
      `/backup/${server.backupId}?tokenType=bot`
    );
